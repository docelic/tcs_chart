...
sub compute_compliance {
  ...
  my %data = ();
  $data{comment} = "$$s{shortname}: $$ti{point_string} = ${\( status2string($$si{compliant}) )}.\n" . ($$si{comment} ? "$$si{comment}\n" : '');
  ...
  if( $$ti{depends_on}) {
    ...
    if( defined $$s{points}{ $dep_point }{compliant}) {
      ...
      $data{comment} .= "TCS requirement for $$ti{point_string} is unknown due to $dep_point = $dep_val.\n";
      #$data{comment_flag} ||= '*'
    }
  }
  $data{must} = $must;
